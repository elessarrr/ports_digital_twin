...
def create_port_layout_chart(berths_data: pd.DataFrame) -> go.Figure:
    ...
    for i, berth in berths_data.iterrows():
        fig.add_trace(go.Scatter(
            x=[i],  # Simple x-positioning
            y=[1],  # Simple y-positioning
            mode='markers+text',
            marker=dict(
                size=berth['max_capacity_teu'] / 1000,  # Size by capacity
                color=berth_type_colors.get(berth['berth_type'], 'gray'),
                symbol='square',
                line=dict(
                    width=2,
                    color='DarkSlateGrey'
            ...
            text=f"{berth['name']}<br>Cranes: {berth['crane_count']}",
            hoverinfo='text',
            name=berth['name']
        ))

...
from ..utils.marine_traffic_fetcher import MarineTrafficFetcher
...
class VesselDataScheduler:
    ...
    def __init__(self, fetcher_callback: Callable[[], dict], marine_traffic_fetcher_callback: Optional[Callable[[], dict]] = None):
        """
        ...
        Args:
            ...
            marine_traffic_fetcher_callback (Optional[Callable[[], dict]]): Optional callback for MarineTraffic data
        """
        ...
        self.marine_traffic_fetcher_callback = marine_traffic_fetcher_callback
    ...
    def _execute_fetch_job(self) -> None:
        ...
        try:
            ...
            # Call the MarineTraffic fetcher callback if it exists
            if self.marine_traffic_fetcher_callback:
                self.logger.info("Executing MarineTraffic data fetch...")
                marine_traffic_result = self.marine_traffic_fetcher_callback()
                if marine_traffic_result:
                    self.logger.info(f"MarineTraffic fetch completed. Vessels found: {len(marine_traffic_result)}")
                else:
                    self.logger.info("MarineTraffic fetch completed with no data.")

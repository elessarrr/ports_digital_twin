...
def main():
    ...
    with tab3:
        ...
        if not vessel_data.empty:
            ...
            # Activity Trend Graph - Enhanced Analytics Dashboard
            st.subheader("📈 Activity Trend Analysis")
            try:
                # Render the vessel analytics dashboard with activity trends
                render_vessel_analytics_dashboard(vessel_data)
            except Exception as e:
                st.error(f"Error rendering activity trend analysis: {str(e)}")
                st.info("Activity trend analysis is temporarily unavailable. Please try refreshing the data.")

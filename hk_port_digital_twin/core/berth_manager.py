...
class BerthStatus(Enum):
    """Enumeration for berth operational status"""
    AVAILABLE = "Available"
    OCCUPIED = "Occupied"
...
class BerthManager:
    ...
    def _initialize_berths(self, berths_config: List[Dict]):
        ...
        for config in berths_config:
            try:
                ...
                self._publish_berth_status(berth.berth_id)  # Publish initial state
    ...
    def allocate_berth(self, berth_id: int, ship_id: str) -> bool:
        ...
        self._publish_berth_status(berth_id)
        return True
    ...
    def get_berth_status(self, berth_id: int) -> Optional[Dict]:
    ...
    def get_all_berth_statuses(self) -> List[Dict]:
        """Get the status of all berths"""
        return [self.get_berth_status(berth_id) for berth_id in self.berths.keys()]

    def _publish_berth_status(self, berth_id: int):
        """Publish berth status to Redis channel and update stored state."""
        status = self.get_berth_status(berth_id)
        if status:
            try:
                # Store the latest status in a hash
                status_to_store = {k: str(v) for k, v in asdict(status).items()} # Ensure all values are strings for hset
                self.redis_client.hset(f"berth:{berth_id}", mapping=status_to_store)

                # Publish a notification to the channel
                self.redis_client.publish("berth_updates", json.dumps({"berth_id": berth_id}))
                logger.info(f"Published status for berth {berth_id} to Redis.")
            except redis.exceptions.RedisError as e:
                logger.error(f"Failed to publish berth status to Redis: {e}")

    def get_berth_utilization(self) -> Dict[str, float]:
        """Get comprehensive berth utilization statistics

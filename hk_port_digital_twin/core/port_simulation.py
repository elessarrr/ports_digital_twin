...
class PortSimulation:
    ...
    def __init__(self, config: Optional[Dict] = None, scenario_name: str = "Normal Operations"):
        ...
        self.simulation_start_datetime = datetime.now()
    ...
    def ai_optimization_process(self):
        ...
        while self.running:
            try:
                ...
                # Run AI optimization using berth allocation optimizer
                optimization_start = self.env.now
                self.berth_optimizer.clear()
                for ship in ai_ships:
                    self.berth_optimizer.add_ship(ship)
                for berth in ai_berths:
                    self.berth_optimizer.add_berth(berth)
                optimization_result = self.berth_optimizer.optimize_berth_allocation(
                    current_time=self.simulation_start_datetime + timedelta(hours=self.env.now)
                )
                
                # Process optimized allocations
                processed_ships = []
                for ship in self.pending_ships:
                    if ship.ship_id in optimization_result.ship_berth_assignments:
    ...
    def _process_ship_traditional(self, ship: Ship):
        ...
        try:
            ...
            berth = self.berth_manager.get_berth_status(berth_id)
    ...
    def _process_ship_ai_optimized(self, ship: Ship, optimization_result: Dict):
        ...
        try:
            ...
            berth = self.berth_manager.get_berth_status(assigned_berth_id)
            while berth and berth.is_occupied:
                yield self.env.timeout(0.1)  # Wait 6 minutes
                berth = self.berth_manager.get_berth_status(assigned_berth_id)  # Refresh berth status
